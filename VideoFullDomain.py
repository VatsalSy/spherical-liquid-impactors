...
def gettingFacets(filename, Savart = False):
...
Savart = False
nGFS = 2750
Ldomain = 16 #float(sys.argv[1])
GridsPerR = 512 #int(sys.argv[2])
nr = int(GridsPerR*Ldomain)
We, Ohd = 250, 0.0025*np.sqrt(2) #float(sys.argv[3]), float(sys.argv[4])
Ohd = Ohd/np.sqrt(We)  
tsnap = 1e-2 #float(sys.argv[5])
# tc = float(sys.argv[6])
Ohs = 1e-5

rmin, rmax, zmin, zmax = [-Ldomain/2, Ldomain/2, 0, 2.05]
lw = 1
...
for ti in range(nGFS):
    ...
    if not os.path.exists(place):
        ...
        if os.path.exists(name):
            ...
            segs = gettingFacets(place, Savart=Savart)
            if (len(segs) == 0):
                ...
                if Savart is True:
                    cntrl1 = ax.imshow(D2, cmap="hot_r", interpolation='Bilinear', origin='lower', extent=[-rminp, -rmaxp, -zmaxp, -zminp], vmax = 0.5, vmin = -3.0)
                ## V
                cntrl2 = ax.imshow(vel, interpolation='Bilinear', cmap="Blues", origin='lower', extent=[rminp, rmaxp, zminp, zmaxp], vmax = 1.0, vmin = 0.0)
                if Savart is True:
                    cntrl2 = ax.imshow(vel, interpolation='Bilinear', cmap="Blues", origin='lower', extent=[rminp, rmaxp, -zmaxp, -zminp], vmax = 1.0, vmin = 0.0)

                ax.set_aspect('equal')
                ax.set_xlim(rmin, rmax)
                ax.set_ylim(zmin, zmax)
                t2 = t/2.
                ax.set_title('$V_0t/D_0$ = %4.3f' % t, fontsize=TickLabel)
